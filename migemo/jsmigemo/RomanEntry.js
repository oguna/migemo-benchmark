...
function romajiToHiragana(romaji) {
    ...
    while (start < romaji.length) {
        ...
        while (upper - lower > 1 && end <= romaji.length) {
            ...
            lower = utils_1.binarySearch(ROMAN_INDEXES, lower, upper, lowerKey);
            if (lower >= 0) {
                lastFound = lower;
            }
            else {
            ...
            upper = utils_1.binarySearch(ROMAN_INDEXES, lower, upper, upperKey);
            if (upper < 0) {
                upper = -upper - 1;
            }
            end++;
...
function findRomanEntryPredicatively(roman, offset) {
    ...
    for (let i = 0; i < 4; i++) {
        ...
        startIndex = utils_1.binarySearch(ROMAN_INDEXES, startIndex, endIndex, startKey);
        if (startIndex >= 0) {
        ...
        endIndex = utils_1.binarySearch(ROMAN_INDEXES, startIndex, endIndex, endKey);
        if (endIndex < 0) {
            endIndex = -endIndex - 1;
        }
...
//# sourceMappingURL=RomanEntry.js.map